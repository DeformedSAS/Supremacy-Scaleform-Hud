...
// Theeto start
var m_IsBotMatch = false;
var m_suprem_gamemode = '';
var m_suprem_gametype = '';
var m_suprem_mapgroups = '';
var m_suprem_flags = '';
	...
	function StartSearch ()
	{
		...
		else
		{
			...
			if(_GetPlayType() === 'listen')
			{
				...
				GameInterfaceAPI.ConsoleCommand( 'suprem_send_gc_message start_listen');
			}
			else
			{
				var searchRegion = GameInterfaceAPI.GetSettingString( 'suprem_matchmaking_search_region_b' );
				var lu = GameInterfaceAPI.GetSettingString( 'cl_mm_lu' );
				
				if(lu == 'lu')
				{
					GameInterfaceAPI.SetSettingString( 'suprem_matchmaking_search_status', 'searching' );
					GameInterfaceAPI.SetSettingString( 'suprem_matchmaking_search_time', '0');
					GameInterfaceAPI.ConsoleCommand( 'suprem_send_gc_message start_search ' + m_suprem_gamemode + ' ' + m_suprem_gametype + ' ' + m_suprem_mapgroups + ' ' + m_suprem_flags + ' ' + searchRegion + ' ' + lu);

					_ShowCancelSearchButton(true, true);
	...
	{                                    
		...
		btnCancel.SetPanelEvent( 'onactivate', function()
		{
			...
			GameInterfaceAPI.ConsoleCommand( 'suprem_send_gc_message stop_search');
			$.DispatchEvent('PanoramaComponent_Lobby_MatchmakingSessionUpdate', 'updated');
			_ShowHideStartSearchBtn(false, true);
			_ShowCancelSearchButton(false, true);
	...
	function _ApplySessionSettings()
	{
		...
		m_suprem_gamemode = gameMode;
		m_suprem_gametype = GetGameType( gameMode );
		m_suprem_mapgroups = selectedMaps;
		m_suprem_flags = gameModeFlags;
